...
model overrides actually take effect. The session leader runs **Opus 4.8**
(`claude-opus-4-8[1m]` in settings.json); subagents do **not** inherit it by
...
- **Default every spawned subagent and workflow `agent()` to `model: 'sonnet'`.**
  ...
  falls back to the Opus leader, which is expensive at fan-out scale).
