...
| `grove list [--short] [--json] [--no-status]` | List all projects. Agents: do not run (see below) |
...
### `grove list`: do not run it in agent context

Do not run `grove list`. Use `grove path <tag>` for one project, or
`git -C <repo>/master worktree list --porcelain` for all of them.

By default `grove list`, `--short` and `--json` scan git status for every project.
On WSL1 that takes 17+ minutes and pins CPU cores. The source
(`/c/work/grove/src/cli/list.rs`) and `grove list --help` say `--no-status` skips
the scan in every output mode. The rule above still holds.
...
3. **Ephemeral/scratch worktrees go through `grove new --ephemeral` / `grove
   ...
   - Expiry is evaluated **lazily** — nothing fires at the expiry instant.
     ...
     happens whenever `grove gc` next runs. A
...
4. **Raw scratch clones go under `<work_dir>/.scratch/` or the session
   scratchpad (`/tmp/claude-*`).** The user-level PreToolUse hook
   `grove-worktree-guard.sh` blocks raw `git worktree add`, `git clone` and
   `gh repo clone` that target a grove `work_dir`, and allows those two
   places. Clones under `.scratch/` have no registry entry and no
   `expires_at`, so `grove gc` applies a default TTL from each unregistered
   directory's mtime. For a rare, deliberate bypass, append
   `# noqa: grove-worktree` to the command.
5. Use `grove path <tag>` or `git -C <repo>/master worktree list --porcelain` to
   resolve tags, not guessed directory names. Do not run `grove list` (see above).
