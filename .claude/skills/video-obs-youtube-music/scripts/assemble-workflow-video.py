...
# Output
OUTPUT = MOVIES_DIR / "multi_agent_workflow_final.mp4"
...
def check_files():
    ...
    missing = []
    for f in [INTRO_VIDEO, MAIN_VIDEO, SONG_1, SONG_2, CARD_1, CARD_2]:
        if not f.exists():
            missing.append(f)

    # These may still be rendering
    optional_missing = []
    for f in [OUTRO_VIDEO]:
        if not f.exists():
...
def assemble():
    ...
    song_1_dur = get_duration(SONG_1)
    ...
    card_2_pos = intro_dur + song_1_dur + 2.0
    ...
    with tempfile.TemporaryDirectory(prefix="obs-assemble-workflow-") as temp_dir:
        ...
        concat_output = temp_dir_path / "concat_video.mp4"
        subprocess.run([
        ...
        overlay_output = temp_dir_path / "overlay_video.mp4"
...
[1:a]asetpts=PTS-STARTPTS,adelay={int(song_1_dur * 1000)}[a2];
[a1][a2]amix=inputs=2:duration=longest,afade=t=out:st={fade_start}:d=5[aout]
"""

        mixed_audio = temp_dir_path / "mixed_audio.m4a"
        subprocess.run([
            'ffmpeg', '-y',
            '-i', str(SONG_1),
            '-i', str(SONG_2),
