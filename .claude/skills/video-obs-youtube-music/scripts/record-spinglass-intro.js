#!/usr/bin/env node
/**
 * Record SpinGlassCA intro at 4K resolution.
 */
...
const HTML_FILE = process.argv[2] || 'intro-multi-agent-swarm.html';
...
const OUTPUT_FILE = path.resolve(
  ...
  path.join(process.env.OBS_OUTPUT_DIR || path.join(os.homedir(), 'Movies'), 'intro_2026-01-21.mp4')
);
const WIDTH = 4096;
const HEIGHT = 2304;
const FPS = 30;
...
async function main() {
  console.log(`Recording SpinGlassCA intro`);
  ...
  try {
    ...
    await page.waitForTimeout(2000); // Let WebGL fully initialize
