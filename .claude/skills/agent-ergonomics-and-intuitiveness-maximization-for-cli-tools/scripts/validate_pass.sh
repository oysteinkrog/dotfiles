...
usage() {
  cat <<'EOF'
Usage: scripts/validate_pass.sh <workspace-dir>

Validates that an audit workspace is internally consistent. Checks: manifest
is valid JSON; every applied:true rec has a matching applied_changes row +
regression test; every applied:false rec in a completed full pass has a
deferred_reason; every score > 700 has evidence; HANDOFF.md exists for
completed passes.

Args:
  <workspace-dir>   Absolute path to the audit workspace. As of the
                    2026-05-09 revision, the convention is the IN-TREE
                    location <target>/agent_ergonomics_audit/. Older runs
                    that used the legacy sibling layout
                    (<target>__agent_ergonomics_audit/) still validate, but
                    you should migrate them in-tree.
...
Exit codes:
  ...
  2  Missing arguments, or workspace directory does not exist (input error,
     distinct from the "checks failed" case so a CI runner can tell them
     apart).

Example:
  scripts/validate_pass.sh /path/to/mytool/agent_ergonomics_audit
EOF
}
