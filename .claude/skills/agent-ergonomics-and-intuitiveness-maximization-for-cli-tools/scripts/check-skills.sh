...
Args:
  <audit-dir>   Absolute path to the audit/ directory inside the in-tree
...
Example:
  scripts/check-skills.sh /path/to/mytool/agent_ergonomics_audit/audit
...
REQUIRED_SKILLS=(
  ...
  multi-pass-bug-hunting multi-model-triangulation ubs dcg agent-mail beads-br beads-bv cass
  idea-wizard gh-cli cc-hooks
)
