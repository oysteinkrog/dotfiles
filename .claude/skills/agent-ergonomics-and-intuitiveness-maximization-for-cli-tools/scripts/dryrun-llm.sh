...
if [ ! -f "$WORKDIR/manifest.stub.json" ]; then
  jq -nc '{schema_version:"1.1.0",tool_name:"dryrun",tool_repo:"/dev/null",audit_workspace:"/dev/null",current_pass:1,passes:[{pass:1,started_at:"2026-01-01T00:00:00Z",target_branch:"main",feature_branch:null,summary:{},artifacts:{}}]}' \
