...
### Symptom: `scaffold-workspace.sh` fails because the in-tree workspace already exists

**Causes.** Resumed pass; or stale debris from an aborted pass. (Note: in this version of the skill `<SIBLING>` resolves to `<target>/agent_ergonomics_audit/` — an in-tree folder, not a sibling directory.)
...
- If a **legacy sibling directory** `<target>__agent_ergonomics_audit/` exists outside the target repo: migrate its contents into `<target>/agent_ergonomics_audit/` (preserve manifest history), record `migrated_from_sibling: true` in the new pass entry, then proceed.

### Symptom: a teammate or older skill version created a branch like `agent-ergonomics-pass-N`

**Recovery.** This skill no longer creates branches. The fix:
- Confirm with the user that the branch can be merged/folded back into `main`.
- Switch to `main` (`git switch main`), merge or rebase the stale branch in (`git merge agent-ergonomics-pass-N`).
- Once the work is on `main`, continue the new pass on `main`. Do NOT start a new pass on the stale auto-branch.
...
### Symptom: target's current branch can't be pushed (no remote configured)

**Recovery.**
- Document in HANDOFF.md that the target has no remote; the branch is local-only.
- The audit workspace is in-tree, so it's already preserved alongside the code in local git history; no separate sibling push needed.
- Ask the user how they want to share the work (configure a remote? `git format-patch` it?).
