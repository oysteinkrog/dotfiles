...
> **Path convention.** Every `audit/...` path below is shorthand for `<SIBLING>/audit/...` — and `<SIBLING>` now resolves to **`<TARGET>/agent_ergonomics_audit/`** (an IN-TREE folder inside the target repo, not a sibling directory). The variable name is legacy; the location is in-tree. Bare `audit/...` reads as workspace-relative throughout this file (and inside the JSONL records the phases produce, where relative paths keep records portable across machines). When you spawn a subagent that touches one of these files, hand it the absolute `<SIBLING>` value (`<TARGET>/agent_ergonomics_audit/`) so it can resolve every reference unambiguously. See `IO-CONTRACTS.md § Path convention` for the full rule.

---

## Phase 0 — Input + Bootstrap

**Inputs.** Target CLI path, mode choice, triangulation appetite, CASS appetite, scope guardrails. **Note:** there is no branch input — this skill always commits to the target's current branch.

**Work.**

> **Workspace location.** `<SIBLING>` in the steps below means **`<TARGET>/agent_ergonomics_audit/`** — a folder INSIDE the target repo. Do NOT create a sibling directory next to the target. Do NOT `git init` a separate workspace. The folder gets committed alongside code changes to the target's current branch. (The variable name `<SIBLING>` is legacy; the location is in-tree.)
...
2. Run `scripts/scaffold-workspace.sh <SIBLING> <target>` where `<SIBLING>` = `<target>/agent_ergonomics_audit/`. Creates the `audit/` tree (regression_tests, agent_simulations, partial, triangulation, .archive) and `tools/` inside `<target>/agent_ergonomics_audit/`, plus the workspace's own README.md. **Do NOT `git init`** — the target's existing git is the system of record. **Do NOT add `agent_ergonomics_audit/` to `.gitignore`** — these artifacts are meant to be committed alongside code. The script writes a populated `audit/manifest.json` seeded with `tool_name`, `tool_repo` (absolute path to target), `audit_workspace` (absolute path to `<target>/agent_ergonomics_audit/`), `current_pass: 1`, and a pass-1 entry containing `started_at`, `target_sha` (from `git rev-parse HEAD` in the target), `target_branch` (from `git rev-parse --abbrev-ref HEAD` in the target — recorded for traceability, not because we'll switch off it), and the artifacts/summary skeleton. (Idempotent — re-running on an already-scaffolded folder exits 0 without overwriting.)
3. Run `scripts/discover-cli.sh <target>` → redirect to `<SIBLING>/audit/phase0_cli.json` (language, build system, binaries, completion-script paths, embedded man pages, env-var prefix conventions). Path resolves to `<target>/agent_ergonomics_audit/audit/phase0_cli.json`.
...
5. Use `scripts/manifest_update.sh <SIBLING> '.passes[-1].mode = "<MODE>"'` to record the mode in the seeded manifest. Don't recreate the manifest from the template — the scaffold already produced a real, schema-valid one. (The legacy `feature_branch` field is no longer set by Phase 0; if present from prior passes, leave it alone.)
6. Write `<SIBLING>/audit/phase0_scope_decision.md` (mode, "must not touch" list, deprecation policies, scope guardrails). See `IO-CONTRACTS.md § audit/phase0_scope_decision.md`.
7. **Branch handling.** Do NOT create a new branch. Do NOT switch off the current branch. If the user's prompt explicitly asked to use a different branch, switch to that branch with their consent and record it in the manifest. If the current branch is something other than `main` because a swarm-mate created an `agent-ergonomics-pass-N` branch from a stale revision, **fold it into `main` first** (with the user's confirmation if conflicts) before continuing — do not stack a new pass on top of a stale auto-branch. See SKILL.md "Branch Policy" for the full rationale.
8. **Workspace handling.** If a sibling `<target>__agent_ergonomics_audit/` already exists from a prior run of the legacy version of this skill, copy/move its contents into `<target>/agent_ergonomics_audit/` (preserve `audit/manifest.json` history) and add a one-line note to the manifest's pass-1 entry: `"migrated_from_sibling": true`. Do not silently abandon prior measurement history.
...
- [ ] Target's current branch recorded in `manifest.json` (`target_branch`) for traceability. **No new branch was created.**
...
**Inputs.** Top-N ranked recommendations. Commits land on the target's **currently checked-out branch** (typically `main`) — Phase 0 did NOT create a new branch; do not create one here.
...
2. **File reservations.** Before editing a shared file (any file mentioned by ≥ 2 recs), use Agent Mail `file_reservation_paths` with `reason="R-NNN"` and `thread_id="agent-ergo-pass<N>-R-NNN"`. This is how we coordinate concurrent work — not via branches.
...
**Be ambitious.** If after applying the first batch the count is < 5 substantive commits (or < 10 for a non-trivial CLI), do not declare Phase 5 done. Pull more recommendations from the ranked list and ship them. The Ambition Bar (SKILL.md § "Ambition Bar") and the verbatim "That's it??" self-prompt exist precisely so you don't stop early. Pull through it.
...
- **Never create a new branch for this skill.** All commits land on the current branch.
...
- One commit per applied rec on the target's **current branch** (commit message: `R-NNN: <short> (closes <surface_id>)`)
...
- [ ] No file deletions in the applied range — verify against the pass's recorded start SHA: `START=$(jq -r '.passes[-1].target_sha' agent_ergonomics_audit/audit/manifest.json) && git diff --diff-filter=D --name-only "$START..HEAD"` should return empty. Per AGENTS.md rule #1, file deletion requires explicit user permission.
- [ ] Commits are on the current branch (typically `main`); no new branch was created.
...
**Inputs.** Modified target on the current branch, the pre-pass `agent_surfaces.jsonl`.
...
**Output artifacts.**
- Additional commits on the current branch addressing fresh-eyes findings
...
3. **Ambition Bar self-check (gate, before landing the plane).** Re-read SKILL.md § "Ambition Bar (the 'That's it??' gate)". If the soft target is unmet (< 5 substantive commits for a tiny CLI; < 10 for non-trivial; or none of the five required surface types added when missing), run the verbatim "That's it??" self-prompt and re-enter Phase 4/5 for one more apply round. After at most one such round, proceed regardless — but persist `<SIBLING>/audit/ambition_bar_check.md` recording: the count of substantive commits, dimensions touched, items deferred, and the user-facing summary that would be served at handoff.
4. **Land the plane (per AGENTS.md):**
   - The audit workspace is in-tree (under `<TARGET>/agent_ergonomics_audit/`), so a single commit + push covers both code changes and audit artifacts. From inside `<TARGET>`:
     ```bash
     git add agent_ergonomics_audit/        # workspace updates (manifest, recommendations.jsonl, etc.)
     git add <changed source files>         # code changes from Phase 5 (already committed if you committed per-rec)
     git commit -m "agent-ergonomics pass <N>: handoff + manifest update"
     git push origin <current-branch>       # typically `main`
     ```
   - The skill never creates an `agent-ergonomics-pass-N` branch and never merges anything itself.
   - Run `br sync --flush-only` if beads were created in the target.
5. **Update manifest.** `pass_N+1_ready: true` if the median uplift is satisfying; otherwise `pass_N+1_ready: false` and `next_pass_focus: <area>`.

**Output artifacts.**
- `audit/HANDOFF.md`
- `audit/ambition_bar_check.md`
- Updated `audit/manifest.json`
- Pushed commits on the target's current branch (covers both code and in-tree workspace artifacts)

**Exit criteria.**
- [ ] HANDOFF.md exists and ends with a "Pass N+1 focus" section.
- [ ] `ambition_bar_check.md` exists and either confirms the soft target was met or records that the self-prompt round was run and what was deferred.
- [ ] All beads filed for queued work.
- [ ] Target's current branch pushed (no new branch was created); the in-tree `agent_ergonomics_audit/` folder is committed alongside code on that same branch (so a single push covers both).
