...
The workflows reference `agent_ergonomics_audit/audit/regression_tests/` (the durable test suite produced by Phase 5). They run on every PR, plus optionally on a schedule for slow drift detection.
    ...
    steps:
      ...
      - name: Capabilities schema pin
        run: |
          ...
          want=$(cat agent_ergonomics_audit/audit/regression_tests/capabilities-golden.json | jq -S .)
      ...
      - name: Run all regression tests
        run: |
          export TOOL_BIN=$(pwd)/target/release/mytool
          failed=0
          for t in agent_ergonomics_audit/audit/regression_tests/R-*.test.sh; do
            echo "::group::$t"
            if bash "$t"; then
              echo "::endgroup::"
            else
      ...
      - name: Upload test artifacts on failure
        ...
        with:
          ...
          path: |
            agent_ergonomics_audit/audit/scorecard*.md
            agent_ergonomics_audit/audit/regression_alerts.md
```
    ...
    steps:
      ...
      - name: Capabilities + footer + verb gates
        run: |
          ...
          bash agent_ergonomics_audit/audit/regression_tests/capabilities-pin.test.sh
      ...
      - name: All regression tests
        run: |
          export TOOL_BIN=$(pwd)/mytool
          for t in agent_ergonomics_audit/audit/regression_tests/R-*.test.sh; do bash "$t" || exit 1; done
```
  ...
  agent-ergo:
    ...
    steps:
      ...
      - name: Pytest regression tests
        run: pytest agent_ergonomics_audit/audit/regression_tests/ -v
  ...
  agent-ergo:
    ...
    steps:
      ...
      - name: Vitest regression tests
        run: bun test agent_ergonomics_audit/audit/regression_tests/
    ...
    steps:
      ...
      - name: Re-score Pass N+1
        run: |
          ...
          # (assumes the audit workspace is committed at <repo>/agent_ergonomics_audit/)
          bash agent_ergonomics_audit/audit/scripts/rescore.sh
...
- **Don't** require the audit workspace to be a separate repo if you can avoid it. Committing `agent_ergonomics_audit/audit/regression_tests/` directly into the target repo is simpler.
