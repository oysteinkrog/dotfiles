...
### Create a new branch for the apply phase (e.g. `agent-ergonomics-pass-N`)

**Reversed from earlier versions of this doc.** The user explicitly detests auto-branching. This skill **never creates a new branch** and never asks about one. All applied changes commit directly to the target's currently checked-out branch (typically `main`). Multi-agent coordination happens via Agent Mail file reservations + Beads, not via branches. If you arrive on a stale auto-branch from a prior version of this skill, fold it into `main` first (with the user's confirmation if conflicts), then continue. See SKILL.md "Branch Policy."

### Create a sibling audit workspace (e.g. `<target>__agent_ergonomics_audit/`)

**Reversed from earlier versions of this doc.** The audit workspace lives **inside the target repo** at `<target>/agent_ergonomics_audit/` and is committed alongside code changes to the same branch. Never `git init` a separate workspace. Never put the workspace in `/tmp/`. Never use a sibling. If a legacy sibling exists from a prior run, migrate its contents into the in-tree folder before continuing.

### Modify the agent_ergonomics_audit/ folder as part of *code-change* commits

The workspace folder is committed alongside code changes, but the work is structured: code edits and audit-artifact writes go in **separate commits**. A Phase 5 commit message like `R-NNN: <short> (closes <surface_id>)` should touch only the source file(s) named in the recommendation. The workspace updates (manifest bump, applied_changes.jsonl append, regression_tests/ addition) go in the next commit.
...
The current branch may be pushed and re-pushed (additive) but never `--force` without authorization. Force-pushing `main` is especially dangerous since it can erase work from concurrent agents.
