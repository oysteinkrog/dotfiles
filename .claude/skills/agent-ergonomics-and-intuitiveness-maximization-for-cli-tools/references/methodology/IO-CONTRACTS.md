...
> **`<SIBLING>` resolves to `<TARGET>/agent_ergonomics_audit/`.** As of the 2026-05-09 revision, the audit workspace lives **inside the target repo** at `<target>/agent_ergonomics_audit/` — NOT in a sibling directory. The variable name `<SIBLING>` is kept for back-compat with subagents and scripts; the location is in-tree. Never read it as "create a sibling directory."
...
When in doubt: if you're telling an agent to read or write a file, use `<SIBLING>/audit/foo` (which resolves to `<TARGET>/agent_ergonomics_audit/audit/foo`). If you're storing a path inside a JSON record that someone else will read later, use `audit/foo`.
...
**Schema version 1.1.0** (current). Changes from 1.0.0:
- Added `passes[*].target_branch` (current branch at start of pass; never switched off).
- `passes[*].feature_branch` is **deprecated** — always `null` in new passes; old manifests may have `agent-ergonomics-pass-N` strings, leave them alone.
- Added `passes[*].summary.{substantive_commits, dimensions_touched, ambition_bar_met, ambition_bar_self_prompt_run}` for the Ambition Bar gate.
- Added `passes[*].artifacts.ambition_bar_check` pointing at `audit/ambition_bar_check.md`.
- `audit_workspace` field now stores the **in-tree** absolute path (`<target>/agent_ergonomics_audit/`); old manifests pointing at a sibling location should be migrated, not silently overwritten.
...
```json
{
  ...
  "audit_workspace": "<absolute path to <target>/agent_ergonomics_audit/ — IN-TREE, never a sibling>",
  ...
  "passes": [
    {
      ...
      "summary": {
        ...
        "substantive_commits": 14,
        "dimensions_touched": 5,
        "ambition_bar_met": true,
        "ambition_bar_self_prompt_run": false,
      ...
      "artifacts": {
        ...
        "ambition_bar_check": "audit/ambition_bar_check.md",
...
**Workspace.** `<TARGET>/agent_ergonomics_audit/` — in-tree; never a sibling
**Target branch.** `<typically 'main'; this skill never creates a new branch and never switches off the current one>`
