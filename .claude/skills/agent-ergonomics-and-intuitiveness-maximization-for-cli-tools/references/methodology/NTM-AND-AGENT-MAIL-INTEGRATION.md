...
> **Path convention.** Wherever this file says `<sibling>` or "sibling workspace," read it as the in-tree audit workspace at `<TARGET>/agent_ergonomics_audit/`. The variable name is legacy; the location is in-tree. The skill no longer creates a sibling directory next to the target.
...
### Step 1 — register the target repo as the audit project

```
ensure_project(project_key=<absolute path to target repo>)
register_agent(project_key, program="claude-code", model="opus-4.7", agent_name="aerg-master")
```

### Step 2 — reserve in-tree audit workspace paths

```
file_reservation_paths(
  project_key=<target repo>,
  agent_name="aerg-master",
  paths=["agent_ergonomics_audit/audit/manifest.json", "agent_ergonomics_audit/audit/HANDOFF.md"],
  ttl_seconds=86400,
  exclusive=true,
...
Each subagent:
1. Reserves its partial output file (`agent_ergonomics_audit/audit/partial/inventory_<subtree>.jsonl`)
...
```bash
# Create per-target in-tree audit workspace dirs
