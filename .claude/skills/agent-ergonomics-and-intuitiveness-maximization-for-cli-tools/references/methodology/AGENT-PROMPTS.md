...
6. Commit on the **current branch** (typically `main`; never create a new branch): `<RECOMMENDATION_ID>: <title> (closes <surface_ids>)`.
...
Exit when applied:true is flipped and the commit is on the current branch.
```
...
- Implement on the same current branch (typically `main`).
...
- [ ] target's current branch pushed (typically `main`); workspace folder committed alongside code
- [ ] no new branch was created (verify `git branch --show-current` matches `target_branch` from manifest)
- [ ] beads created for queued work
- [ ] manifest updated with `pass_N+1_ready=<bool>` (literal field name; `<bool>` is the value)
- [ ] ambition_bar_check.md present and either records target met or deferrals
```
