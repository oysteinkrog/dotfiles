...
- [Lessons from the 2026-05-07 bv dogfood](#lessons-from-the-2026-05-07-bv-dogfood)
- [Pre-flight (before the user's "go")](#pre-flight-before-the-users-go)
- [During the audit](#during-the-audit)
- [After the audit](#after-the-audit)
- [Things to watch for (lessons from synthetic smokes)](#things-to-watch-for-lessons-from-synthetic-smokes)
- [What the synthetic smokes did NOT cover (what to expect from a real run)](#what-the-synthetic-smokes-did-not-cover-what-to-expect-from-a-real-run)
- [If the audit goes wrong](#if-the-audit-goes-wrong)
...
> **Branch + workspace policy (read first).** The skill **never** creates a new branch — all applied changes commit directly to the target's currently checked-out branch (typically `main`). The audit workspace is **inside** the target repo at `<target>/agent_ergonomics_audit/`, NOT a sibling directory. Every `<sibling>` placeholder in the commands below resolves to `<target>/agent_ergonomics_audit/`. See SKILL.md "Branch Policy" and "Workspace Policy" callouts.
...
8. **Validate at every transition.**
   ...
   - After Phase 5: `git -C <target> log --oneline -n 30` (one commit per applied rec, on the **current branch**, typically `main` — verify no new branch was created via `git -C <target> branch --show-current`)
