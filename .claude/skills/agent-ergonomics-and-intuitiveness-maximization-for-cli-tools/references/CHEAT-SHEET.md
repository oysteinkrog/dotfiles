...
- `full` — **default for any "apply" intent.** audit + apply + re-score + tests; commits to current branch (typically main, never a new branch); in-tree workspace at `<target>/agent_ergonomics_audit/`; Ambition Bar self-prompt before declaring done
...
9. **NEVER create a new branch for this skill** — always commit directly to the current branch (typically `main`)
...
## The audit workspace layout (in-tree, NOT a sibling)

```
<target>/  (current branch, typically main; NEVER a new branch)
├── (your code diffs from Phase 5)
├── agent_ergonomics_audit/      ← INSIDE the target repo, committed to the same branch
│   ├── audit/
...
│   │   ├── ambition_bar_check.md    ← Phase 10 gate evidence
│   │   ├── scorecard*.md, heatmap.svg, uplift_diff.md
│   │   ├── regression_tests/        ← R-NNN__*.test.{sh,rs,py,ts}
│   │   ├── agent_simulations/{pre,post}_pass_<N>/
│   │   └── HANDOFF.md
│   └── tools/
└── (the rest of the target repo)
```

Code changes commit to the current branch alongside workspace updates. **No sibling. No new branch.** Legacy variable `<SIBLING>` resolves to `<target>/agent_ergonomics_audit/`.
...
2. `scripts/scaffold-workspace.sh <SIBLING> <target>` where `<SIBLING>` = `<target>/agent_ergonomics_audit/` → create `audit/` (and `tools/`) inside the target; do NOT `git init` the workspace
...
```
schema_version: 1.1.0   # 1.1.0 (2026-05-09): in-tree workspace, no auto-branch, Ambition Bar fields
