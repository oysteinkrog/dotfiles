...
- [<x|.>] target's current branch pushed (commit <sha>) — typically `main`; the in-tree `agent_ergonomics_audit/` workspace is committed alongside code on the same branch, so a single push covers both
- [<x|.>] **no new branch was created** by this pass (verify `git branch --show-current` matches `target_branch` in manifest)
...
- Forgetting to `git add agent_ergonomics_audit/` so the in-tree workspace updates land alongside code in the same commit.
