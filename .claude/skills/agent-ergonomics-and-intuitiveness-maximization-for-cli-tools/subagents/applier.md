...
description: Phase 5 — implements one recommendation by committing directly to the target's currently checked-out branch (typically main). Never creates a branch. Smallest change that closes failing dims; preserves all existing functionality.
---

# Applier

You implement ONE recommendation by committing directly to the target repo's **currently checked-out branch** (typically `main`). **Never** create a new branch. **Never** switch branches. The audit workspace lives in-tree at `<TARGET>/agent_ergonomics_audit/` (your `<SIBLING>` input resolves there), not in a sibling directory.

## Inputs

- `<RECOMMENDATION_ID>` — your assigned rec
- `<TARGET>` — the target repo absolute path
- `<SIBLING>` — audit workspace root (absolute path); equals `<TARGET>/agent_ergonomics_audit/` in this version of the skill. All `audit/...` paths below are relative to `<SIBLING>`.
- `<TARGET_SHA>` — the SHA at which you read `recommendations.jsonl` (informational; you commit on top of HEAD, whatever HEAD is)
- `<SIBLING>/audit/recommendations.jsonl` — find your rec by recommendation_id
- `<TARGET>/AGENTS.md` — RESPECT IT

> **No `<FEATURE_BRANCH>` input.** Earlier versions of this subagent took a feature-branch argument and ran `git switch -c`. That has been removed. You commit on whatever branch the target repo is currently checked out to. If that branch isn't `main`, **do not switch** — but record a warning in the per-step ledger so the orchestrator can fold it back later.
...
The code change lives in `<TARGET>` (the target repo's **current branch** —
typically `main`, never a new branch) and the regression test lives in
`<SIBLING>/audit/regression_tests/`. In this version of the skill they are the
**same git repo** — `<SIBLING>` is `<TARGET>/agent_ergonomics_audit/`, an
in-tree folder. Commit the source change here; the workspace updates
(`recommendations.jsonl` flip, `applied_changes.jsonl` append, regression test
file) get committed by the orchestrator in batch at end-of-pass (Phase 10) so
each Phase-5 commit stays scoped to its source diff.
