...
- `<SIBLING>` — audit workspace root (absolute path); convention is the in-tree
  `<TARGET_SKILL>/agent_ergonomics_audit/` folder. All `audit/...` paths below
  are relative to `<SIBLING>`.
...
Exit when audit artifacts are produced in `<TARGET_SKILL>/agent_ergonomics_audit/`.
