...
**Target branch.** <typically `main`; this skill never creates a new branch>; status: <pushed|local-only>
**Workspace location.** `<TARGET>/agent_ergonomics_audit/` (in-tree; committed alongside code on the same branch)
...
- [ ] target's current branch pushed (commit <sha>) — typically `main`; the workspace folder `agent_ergonomics_audit/` is committed alongside code in the same branch
- [ ] **no new branch was created** by this pass — verify `git branch --show-current` matches the manifest's `target_branch`
- [ ] beads created for queued work (count: <N>)
- [ ] manifest updated with `pass_<N+1>_ready: <bool>`
- [ ] `audit/ambition_bar_check.md` records target met or deferrals (full mode only)
