...
How words and sentences are priced, what gates, the second-language testing and
how the gate is wired are all in `tool/README.md`. The one thing you may need
here: put terms that should cost nothing in `<repo>/.plainlang/glossary.txt`, one
per line. The scorer finds it by walking up from the working directory.
...
`bash selftest.sh` proves the whole gate works on the current machine.
